...
def gen_embeddings(args, vocab):
    ...
    logger.info('Embeddings: {} x {}'.format(vocab.n_words, args.emb_dim))
    if args.emb_file is not None:
        logger.info('Loading embedding file: {}'.format(args.emb_file))
        pre_trained = 0
        for line in open(args.emb_file).readlines():
            ...
            if len(sp) == args.emb_dim + 1:
                ...
                print(sp[0])
        logger.info("Pre-trained: %d (%.2f%%)" % (pre_trained, pre_trained * 100.0 / vocab.n_words))
    return embeddings


def share_embedding(args, vocab, pretrain=True):
    embedding = Embeddings(vocab.n_words, args.emb_dim, padding_idx=constants.MAP_VOCAB['PAD'])
