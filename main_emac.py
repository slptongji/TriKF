...
from dataloader_emac import prepare_emac_data_seq, prepare_data_seq, make_infinite
from common import *
from eval.eval import read_file, calc_distinct, calc_bleu, calc_meteor
...
def load_params():
    ...
    parser.add_argument("--data_dir", type=str, default="data/EMAC",
...
def evaluate(model, data, eval_type='valid', d_type='eqt', max_dec_step=30, output_file=None):
    ...
    for _, batch in pbar:
        l, p, bce_prog, acc_prog, top_preds, comet_res, pol_acc_prog = model.train_one_batch(batch, 0, train=False, d_type=d_type)
        loss.append(l)
        ppl.append(p)
        bce.append(bce_prog)
        acc.append(acc_prog)
...
def test(args, model, test_set, d_type='eqt'):
    model.is_eval = True
    loss_test, ppl_test, bce_test, acc_test, results, pol_acc_test = evaluate(model, test_set, eval_type='test', d_type=d_type, max_dec_step=50)
    output_file = os.path.join(args.output_dir, 'results.txt')
    with open(output_file, 'w') as f:
        f.write("Loss: {:.4f}\tPPL: {:.4f}\tBCE: {:.4f}\tAccuracy: {:.4f} {:.4f}\n".format(loss_test, ppl_test, bce_test, acc_test, pol_acc_test))
        for r in results:
            f.write(r)
...
def eval_metrics(file_path):
    refs, cands, _, _ = read_file(file_path)
...
def main():
    ...
    emma_test_set, vocab, _ = prepare_emac_data_seq(args, vocab)

    model = MyModel(args=args, vocab=vocab, decoder_number=dec_num)
    ...
    if args.do_test:
        ...
        model.epoch = 100
        with torch.no_grad():
            test(args, model, emma_test_set, d_type='emma')
